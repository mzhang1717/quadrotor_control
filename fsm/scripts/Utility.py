...
def getWaypoints(waypoints):
    ...
    for next_pose in waypoints:
        StampedPose_final                 = PoseStamped() #Construct a StampedPose MSG        
        StampedPose_final.header.frame_id = "/world" #Frame of ref that the trajectory is formualted in
        StampedPose_final.header.stamp    = rospy.Time.now()
        StampedPose_final.pose            = Pose(next_pose.position, next_pose.orientation) #The pose
        trajectory.append(StampedPose_final) ## Append the time staped pose to the trajectory to follow
    
    return trajectory
		


## getTrajectory() is written for testing
## Author: Mingfeng
## Jan. 13, 2014
## After modification, it only returns the final destination point

def getTrajectory(currentPose, TargetPose, tolerance = 0.01): 
    trajectory = []
    
    quat    = Quaternion(TargetPose.orientation.x, TargetPose.orientation.y, TargetPose.orientation.z, TargetPose.orientation.w)
    StampedPose_final                 = PoseStamped() #Construct a StampedPose MSG        
    StampedPose_final.header.frame_id = "/world" #Frame of ref that the trajectory is formualted in
    StampedPose_final.header.stamp    = rospy.Time.now()
    StampedPose_final.pose            = Pose(Point(TargetPose.position.x, TargetPose.position.y, TargetPose.position.z),quat) #The pose
    trajectory.append(StampedPose_final) ## Append the time staped pose to the trajectory to follow
    return trajectory
...
###    print "Euclidean Distance to Target: " , distance_to_target 
    ...
    ### Modified by Mingfeng, Jan. 13, 2014
    #delta   = numpy.amax([abs(x_final-x_init),abs(y_final-y_init),abs(z_final-z_init)])
    #if delta<tolerance:
